...
The fields from ES relate to the search results directly. The other fields are regular fields used to display a video information on the screen.

The 'results' key will contain an empty list if there is no search results.
</br></br>
...
| pages               | integer | The number of pages (chunks) returned                 |
| page                | integer | The page number in this set of results                |
| per_page            | integer | the number of entries per page                        |
| total               | integer | The total number of results found                     |
| using_elasticsearch | boolean | Whether elastic search was used to return the results |
| results             | list    | individual entries, which make up search results      |
</br></br>


> [!NOTE]
> There is a maximum of 100 results per page.
> Setting 'per_page' higher than 100 will not return an error. It will default to 100 per page.
</br></br>


> [!NOTE]
> Viewing an invalid page number does not return an error.
> Instead this will return as normal, but the results field will be an empty list.
</br></br>
...
| --------------- | ------- | ------------- | --------------------------------------------- |
| id              | integer | ElasticSearch | The video's ID                                |
| score           | float   | ElasticSearch | The search relevance score                    |
| title           | string  | ElasticSearch | The video's title                             |
| highlights      | object  | ElasticSearch | Fields showing how a video matched the search |
| video_id        | integer | Database      | The video's ID                                |
| name            | string  | Database      | The video's name                              |
| description     | string  | Database      | The video's description                       |
| duration        | integer | Database      | Video duration in seconds                     |
| chapter_markers | string  | Database      | Chapters in the video                         |
| thumbnail       | string  | Database      | URL to the video thumbnail                    |
| bible_character | string  | Database      | Bible characters in the video                 |
| location        | string  | Database      | Locations in the video                        |
| scriptures      | string  | Database      | Scriptures in the video                       |
| speaker         | string  | Database      | Speakers in the video                         |
| tags            | string  | Database      | Tags in the video                             |
| watched         | boolean | Database      | Watch status for the current user             |
</br></br>


Each result has a 'highlights' field. This is information added by ElasticSearch about the search results.

This includes snippets of information showing why this search result was returned.

The fields within 'highlights' will vary, depending on what matched the search query. The content will enclose matched search terms in <em></em> tags to emphasise the match.

Highlights fields include:

| Field           | Type            | Description                        |
| --------------- | --------------- | ---------------------------------- |
| title           | list of strings | Matches in the video's title       |
| transcript      | list of strings | Matches in the video transcript    |
| description     | list of strings | Matches in the video's description |
| tags            | list of strings | Matches in the video's tags        |
| location        | list of strings | Matches in the video's locations   |
| bible_character | list of strings | Matches in the video's characters  |
| scriptures      | list of strings | Matches in the video's scriptures  |
| speaker         | list of strings | Matches in the video's speakers    |
</br></br>
