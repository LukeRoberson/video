...
class VideoContextMenu {
    ...
    private isMenuOperation: boolean = false;
...
class UrlTimeHandler {
    ...
    private createSnippetIndicator(startTime: number, endTime: number): void {
        ...
        this.player.on('play', () => {
            if (this.snippetIndicator) this.snippetIndicator.style.display = 'none';
        });
        this.player.on('pause', () => {
            if (this.snippetIndicator) this.snippetIndicator.style.display = '';
        });
    }
 ...
 */
function handleTheatreMode(player: VideoJsPlayer, container: HTMLElement): void {
     ...
     */
    function enterTheatreMode(): void {
        // Store original position in DOM
        originalParent = wrapper.parentElement as HTMLElement;
